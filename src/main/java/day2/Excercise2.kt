...
fun main() {
    val sum = input2.split("\n")
        .asSequence()
        .sumOf { processLine2(it) }

    println(sum)
}

data class MinimumBalls(var redMin: Int = 0, var greenMin: Int = 0, var blueMin: Int = 0)
fun processLine2(line: String): Int {
    val balls = MinimumBalls()
    val parts = line.split(":")
    val gameId = parseInt(parts[0].split(" ")[1])
    val violates = parts[1].split(";").map { draw ->
        val all12 = draw.trim().split(", ").map { oneColorBalls ->
            val count = parseInt(oneColorBalls.split(" ")[0])
            val color = oneColorBalls.split(" ")[1]

            val bool = if (count > limits[color]!!) {
                true
            } else {
                false
            }
            ...
            if (true) {
                if (color == "blue") {
                    balls.blueMin = max(count, balls.blueMin)
                } else if (color == "green") {
                    balls.greenMin = max(count, balls.greenMin)
                } else if (color == "red") {
                    balls.redMin = max(count, balls.redMin)
                }
            }
    ...
    return balls.greenMin * balls.redMin * balls.blueMin
}


const val input2 = """Game 1: 1 green, 1 blue, 1 red; 1 green, 8 red, 7 blue; 6 blue, 10 red; 4 red, 9 blue, 2 green; 1 green, 3 blue; 4 red, 1 green, 10 blue
