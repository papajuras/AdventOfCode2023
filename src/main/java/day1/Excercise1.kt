...
fun main() {
    ...
    val pairsAsStrings: List<String> = input1.split("\n").map { line ->
        val letters = line.split("")
        val integers: List<Int> = letters.subList(1, letters.size - 1)
            .filter { it.toCharArray()[0].isDigit() }
            .map { parseInt(it) }
    ...
    val result = pairsAsStrings.map { parseInt(it) }.sum()

    println(result)
}

const val input1 = """dqfournine5four2jmlqcgv
7ggzdnjxndfive
twofive4threenine
