...
val ONE = Pair("one", 1)
val TWO = Pair("two", 2)
val THREE = Pair("three", 3)
val FOUR = Pair("four", 4)
val FIVE = Pair("five", 5)
val SIX = Pair("six", 6)
val SEVEN = Pair("seven", 7)
val EIGHT = Pair("eight", 8)
val NINE = Pair("nine", 9)
...
fun findFirstAndLastDigit(line: String): String {
    val indexOfFirstWrittenDigit: Pair<Int, Pair<String, Int>> = DIGITS.map { digit ->
        val firstIndexOfDigit = line.indexOf(digit.first)
        Pair(firstIndexOfDigit, digit)
    }.filter { it.first != -1 }.sortedBy { it.first }.firstOrNull() ?: Pair(9999999, Pair("asd", -1))
    val indexOfFirstDigit: Pair<Int, Pair<String, Int>> = DIGITS.map { digit ->
        val firstIndexOfDigit = line.indexOf("" + digit.second)
        Pair(firstIndexOfDigit, digit)
    }.filter { it.first != -1 }.sortedBy { it.first }.firstOrNull() ?: Pair(9999999, Pair("asd", -1))


    val indexOfLastWrittenDigit: Pair<Int, Pair<String, Int>> = DIGITS.map { digit ->
        val firstIndexOfDigit = line.lastIndexOf(digit.first)
        Pair(firstIndexOfDigit, digit)
    }.filter { it.first != -1 }.sortedBy { -it.first }.firstOrNull() ?: Pair(-1, Pair("asd", -1))
    val indexOfLastDigit: Pair<Int, Pair<String, Int>> = DIGITS.map { digit ->
        val firstIndexOfDigit = line.lastIndexOf("" + digit.second)
        Pair(firstIndexOfDigit, digit)
    }.filter { it.first != -1 }.sortedBy { -it.first }.firstOrNull() ?: Pair(-1, Pair("asd", -1))

    val first =
        "" + listOf(indexOfFirstWrittenDigit, indexOfFirstDigit).sortedBy { it.first }.map { it.second.second }.first()
    val second =
        "" + listOf(indexOfLastWrittenDigit, indexOfLastDigit).sortedBy { -it.first }.map { it.second.second }.first()

    return first + second
}

val input2 = """twofive4threenine
