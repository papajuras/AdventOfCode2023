...
data class AsteriskLocation(val x: Int, val y: Int)
data class NumberLocation(val row: Int, val start: Int, val end: Int, val value: Int)

val asteriskNumberMap = mutableMapOf<AsteriskLocation, MutableSet<NumberLocation>>()
fun main() {
    var sum = 0
    val lines = input1.split("\n")

    val emptyLIne = (1..142).map { "." }.joinToString("")
    val paddedLines = listOf(emptyLIne) + lines.map { (".$it.") } + listOf(emptyLIne)

    println(paddedLines)

    val charactersMatrix: List<List<String>> = paddedLines.map {
        it.split("").subList(1, paddedLines[0].length + 1)
    }

    for (rowNumber in 1..charactersMatrix.size - 1) {
        var numberStart = -1
        var numberEnd = -1
        for (characterNumber in 1..charactersMatrix[rowNumber].size - 1) {
            if (charactersMatrix[rowNumber][characterNumber].toCharArray()[0].isDigit()) {
                if (numberStart == -1) {
                    numberStart = characterNumber
                    numberEnd = characterNumber
                } else {
                    numberEnd = characterNumber
                }
            } else {
                if (numberStart == -1) {
                    ...
                    saveAllAsterisksAroundNumber(charactersMatrix, rowNumber, numberStart, numberEnd)
                }
    ...
    val sum1 = asteriskNumberMap
        .filter { it.value.size == 2 }
        .map { it.value.map { it.value }.reduce(Int::times) }
        .sum()

    println(sum1)
}

fun saveAllAsterisksAroundNumber(
    charactersMatrix: List<List<String>>, rowNumber: Int, numberStart: Int, numberEnd: Int
) {
    // check row above
    for (i in numberStart ..numberEnd ) {
        if (isAsterisk(charactersMatrix[rowNumber - 1][i])) {
            val asteriskLocation = AsteriskLocation(i, rowNumber - 1)
...
private fun saveLocation(
    asteriskLocation: AsteriskLocation,
    rowNumber: Int,
    numberStart: Int,
    numberEnd: Int,
    charactersMatrix: List<List<String>>
) {
    if (asteriskNumberMap.containsKey(asteriskLocation)) {
        asteriskNumberMap[asteriskLocation]?.add(
            NumberLocation(
                rowNumber,
                numberStart,
                numberEnd,
                parseInt(charactersMatrix[rowNumber].subList(numberStart, numberEnd + 1).joinToString(""))
            )
        )
    } else {
        asteriskNumberMap.put(
            asteriskLocation, mutableSetOf(
                NumberLocation(
                    rowNumber,
                    numberStart,
                    numberEnd,
...
fun isAsterisk(s: String): Boolean {
    return s == "*"
}
