...
fun main() {
    ...
    val paddedLines = listOf(emptyLIne) +
            lines.map { (".$it.") } +
            listOf(emptyLIne)
        ...
        for (characterNumber in 1..charactersMatrix[rowNumber].size-1) {
            if (charactersMatrix[rowNumber][characterNumber].toCharArray()[0].isDigit()) {
                ...
                if (numberStart == -1) {
                    ...
                    if (checkIfIsPartNumber(charactersMatrix, numberStart, numberEnd, rowNumber)) {
                        val partNumber =
...
fun checkIfIsPartNumber(charactersMatrix: List<List<String>>, numberStart: Int, numberEnd: Int, rowNumber: Int): Boolean {
    var isPartNumber = false
    // check row above
    for (i in numberStart-1..numberEnd+1) {
        isPartNumber = isPartNumber || isSpecialCharacter( charactersMatrix[rowNumber-1][i])
    }
    // check row below
    for (i in numberStart-1..numberEnd+1) {
        isPartNumber = isPartNumber || isSpecialCharacter( charactersMatrix[rowNumber+1][i])
    ...
    for (i in rowNumber-1..rowNumber+1) {
        isPartNumber = isPartNumber || isSpecialCharacter( charactersMatrix[i][numberStart-1])
    }
    // check column on the right
    for (i in rowNumber-1..rowNumber+1) {
        isPartNumber = isPartNumber || isSpecialCharacter( charactersMatrix[i][numberEnd+1])
    }

    return isPartNumber
}

fun isSpecialCharacter( s: String): Boolean {
    return !s.toCharArray()[0].isDigit() && s != "."
}
